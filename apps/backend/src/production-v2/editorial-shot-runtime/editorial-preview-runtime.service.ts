...
import { renderEditorialPreview } from './runtime.js';
import { EDITORIAL_PREVIEW_RENDER_CONFIG } from './render-config.js';
import { readEditorialSidecar } from './store.js';

@Injectable()
export class EditorialPreviewRuntimeService {
  private readonly inFlight = new Set<string>();

  constructor(private readonly repo: PgCropReviewRepository) {}

  state(tenantId: string, sessionId: string) {
    return readEditorialSidecar({ tenantId, sessionId, previewVersion: EDITORIAL_PREVIEW_RENDER_CONFIG.previewVersion });
  }
  ...
  async requestRender(input: { tenantId: string; sessionId: string; assetId: string }): Promise<{
    ...
    if (existing?.status === 'READY') {
      return { ok: true, ffmpegSpawned: false, previewStatus: 'READY' };
    }
    ...
    const work = renderEditorialPreview({
