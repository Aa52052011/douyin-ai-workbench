...
import { renderDynamicPreview } from './runtime.js';
import { DYNAMIC_PREVIEW_RENDER_CONFIG } from './render-config.js';
import { readDynamicSidecar } from './store.js';

@Injectable()
export class DynamicPreviewRuntimeService {
  private readonly inFlight = new Set<string>();

  constructor(private readonly repo: PgCropReviewRepository) {}

  state(tenantId: string, sessionId: string) {
    return readDynamicSidecar({ tenantId, sessionId, previewVersion: DYNAMIC_PREVIEW_RENDER_CONFIG.previewVersion });
  }
  ...
  async requestRender(input: { tenantId: string; sessionId: string; assetId: string }): Promise<{
    ...
    const work = renderDynamicPreview({
