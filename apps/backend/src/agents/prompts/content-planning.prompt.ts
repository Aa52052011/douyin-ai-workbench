...
Topic.contentPillar 必须仍使用账号定位里已有的支柱名称，不得发明新支柱。可用 title / contentAngle / reason 体现 Strategy 的 pillar、creativeAngles、value propositions。
...
3. contentPillar 必须使用定位里已有的支柱名称，不得发明新支柱。
...
- 不要机械复制历史内容，不要因为一两个信号完全改变账号定位或 Strategy。
