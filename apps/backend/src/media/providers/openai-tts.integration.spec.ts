...
import { isOpenAiTtsConfigured } from '../tts/tts-config.js';
import { LocalStorageProvider } from '../storage/local-storage.provider.js';
import { StorageService } from '../storage/storage.service.js';
import { OpenAiTtsProvider } from './openai-tts.provider.js';

const enabled = process.env.RUN_REAL_TTS_TESTS === 'true' && isOpenAiTtsConfigured();
...
describe.skipIf(!enabled)('OpenAiTtsProvider real integration', () => {
  let root: string;
  let previousRoot: string | undefined;

  beforeEach(() => {
    previousRoot = process.env.MEDIA_STORAGE_ROOT;
    root = mkdtempSync(path.join(os.tmpdir(), 'acf-real-tts-'));
  ...
  it('synthesizes a short wav/mp3 without leaking secrets', async () => {
    ...
    const result = await provider.synthesize({
      ...
      speed: 1,
      language: 'zh-CN',
      clientRequestId: 'real-tts-integration',
    });
