...
import { isOpenAiTtsConfigured } from '../src/media/tts/tts-config.js';
...
describe.skipIf(!enabled)('Real TTS + FFmpeg pipeline (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.MEDIA_STORAGE_ROOT = path.join(os.tmpdir(), `acf-real-tts-e2e-${process.pid}`);
  ...
  it('completes a real mp4 from real TTS through finalize', async () => {
    const email = `tts-${suffix()}@example.com`;
    const user = await request(app.getHttpServer())
      .post('/auth/register')
      .send({ email, password: 'password1', name: 'TtsUser' })
      .expect(201);
    ...
    expect(JSON.stringify(done.body)).not.toContain('TTS_API_KEY');
    ...
    const mp4Path = path.join(process.env.MEDIA_STORAGE_ROOT!, 'real-tts-probe.mp4');
