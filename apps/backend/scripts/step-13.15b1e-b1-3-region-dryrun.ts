...
async function analyzeAsset(prisma: PrismaClient, storage: LocalStorageProvider, assetId: string) {
  ...
  try {
    ...
    return {
      ...
      borderCandidates: merged.borderCandidates,
      emptyRegionCandidates: merged.emptyRegionCandidates,
      topStructuredStripCandidates: merged.topStructuredStripCandidates,
      warnings: merged.warnings,
      uniqueSampleCount: merged.frameStatsAggregate?.sampleCountUsable,
    };
  } finally {
...
async function main() {
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ newVideoWallMs: neu.wallMs, oldVideoWallMs: old.wallMs, note: "same decode as B1.2 plus CPU heuristics" }, null, 2),
    );
  } finally {
