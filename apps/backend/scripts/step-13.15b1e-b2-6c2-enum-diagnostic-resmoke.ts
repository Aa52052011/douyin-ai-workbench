/**
 * B2-6C2 3-frame enum-diagnostic re-smoke. Exactly 1 UI_STRUCTURE inference, 120s.
 */
...
import { buildEnumContractDiagnosis } from '../src/production-v2/visual-semantic/runtime/b2-6c0-enum-diagnosis.js';
...
import { buildUiStructureUserPrompt, PROMPT_MODULES_MULTI_FRAME } from '../src/production-v2/visual-semantic/runtime/ui-structure-prompt.js';
...
try {
  if (!cfg.apiKey || !cfg.baseUrl) {
    ...
    const cacheDir = await mkdtemp(path.join(os.tmpdir(), 'acf-b26c2-cache-'));
    temps.push(cacheDir);
    try {
      ...
      if (!row?.storageKey) {
        ...
        const work = await mkdtemp(path.join(os.tmpdir(), 'acf-b26c2-mat-'));
        temps.push(work);
        ...
        if (!videoScan.ok) {
          ...
          selectedIds = representative.selected.map((item) => item.frameId);
          const b26bExpected = ['semantic-frame:0', 'semantic-frame:20015', 'semantic-frame:35027'];
          writeJson('representative-selection/deterministic-comparison.json', {
            b26bSelectedIds: b26bExpected,
            c2SelectedIds: selectedIds,
            match: selectedIds.length === b26bExpected.length && selectedIds.every((id, i) => id === b26bExpected[i]),
          });
          ...
          await withSemanticFrames(
            ...
            async (prep, scope) => {
              ...
              writeJson('call-a/request-summary.json', {
                requestId: 'b2-6c2-call-a-ui-structure',
                model,
...
if (callA.result) {
  ...
  writeJson('semantic-calibration/browser-result.json', { frames: [...browserFrames], count: browserFrames.size, status: browserStatus });
...
writeJson('latency-summary.json', {
  ...
  b26a: 'TIMEOUT>=150000',
  b26b: 52741,
  b26c2: callA.latencyMs,
  assessment: latencyAssessment,
  runtimeStability: callA.stopReason === 'PROVIDER_TIMEOUT' || callA.latencyMs == null ? 'NOT_CONFIRMED' : 'RECONFIRMED',
});
writeJson('usage-summary.json', {
  usage: callA.result?.usage ?? null,
  costStatus: callA.result?.usage?.costStatus ?? 'UNPRICED',
});
writeJson('latency/historical-comparison.json', {
  b26: { frames: 6, latencyMs: 130019 },
  b26a: { frames: 6, latencyMs: null, timeout: true, timeoutMs: 150000 },
  b26b: { frames: 3, latencyMs: 52741 },
  b26c2: { frames: framesSent, latencyMs: callA.latencyMs, timeoutMs: B2_6B_CALL_A_TIMEOUT_MS, stopReason: callA.stopReason || null },
});
writeJson('latency/workload-reduction-result.json', {
  WorkloadReductionLatency: workload,
  REAL_3_FRAME_UI_STRUCTURE_RUNTIME:
    callA.stopReason === 'PROVIDER_TIMEOUT' || callA.latencyMs == null ? 'NOT_CONFIRMED' : 'RECONFIRMED',
  LatencyAssessment: latencyAssessment,
});
writeJson('latency/usage-summary.json', {
  usage: callA.result?.usage ?? null,
  costStatus: callA.result?.usage?.costStatus ?? 'UNPRICED',
});
...
const runtimeStability = callA.stopReason === 'PROVIDER_TIMEOUT' || callA.latencyMs == null ? 'NOT_CONFIRMED' : 'RECONFIRMED';
const semanticCore =
  pipelinePass(callA) &&
  ...
  leak !== 'INVALID';
const gate = !pipelinePass(callA) || leak === 'INVALID' || identity === 'FAIL' || productUiCalibration === 'FAIL' || callA.stopReason === 'PROVIDER_TIMEOUT'
  ? 'FAIL'
  : productUiCalibration === 'PASS' && latencyAssessment !== 'HIGH'
    ? 'PASS'
...
writeJson('files-changed.json', {
  step: '13.15B-1E-B2-6C2',
  files: [
    'apps/backend/scripts/step-13.15b1e-b2-6c2-enum-diagnostic-resmoke.ts',
...
if (gate === 'FAIL' || privacyBlocked || callA.calls > 1) {
  process.exitCode = 1;
}
