...
function candidateTable(merged: ReturnType<typeof mergeFrameAnalysis>) {
  return (merged.cropGeometryCandidates ?? []).map((c) => ({
    type: c.type,
    sourceRect: c.sourceRect,
    retainedArea: c.retainedAreaRatio,
    occupancy: c.outputOccupancy,
    detailScale: c.effectiveScale,
    ...
    semanticUnconfirmed: c.semanticUnconfirmed ?? false,
    platformConflicts: c.platformConflicts,
  }));
}
...
async function analyzeAsset(prisma: PrismaClient, storage: LocalStorageProvider, assetId: string) {
  ...
  try {
    ...
    return {
      ...
      cropScoringVersion: merged.cropScoringVersion,
      ranking: merged.cropGeometryRanking,
      rankingNote: "geometry candidate ranking is not asset usage eligibility",
      candidates: candidateTable(merged),
      warnings: merged.warnings,
      wallMs: Date.now() - started,
      cropComputeMs: frames.attach.cropComputeMs,
    };
  } finally {
...
async function main() {
  ...
  try {
    ...
    writeFileSync(path.join(outDir, "image-dryrun.json"), JSON.stringify({ images, note: "geometry candidates only; not Content #1 suitability" }, null, 2));
    writeFileSync(
      path.join(outDir, "performance-sanity.json"),
      JSON.stringify(
        {
          newVideoWallMs: neu.wallMs,
          newVideoCropComputeMs: neu.cropComputeMs,
          oldVideoWallMs: old.wallMs,
          oldVideoCropComputeMs: old.cropComputeMs,
          extraFfmpegDecode: false,
        },
        null,
        2,
      ),
...
main().catch(() => {
  writeFileSync(path.join(outDir, "content01-new-video-dryrun.json"), JSON.stringify({ status: "NOT_RUN", reason: "script_error" }, null, 2));
});
